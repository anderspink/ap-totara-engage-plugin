...
namespace engage_anderspink\totara_catalog;
...
use dml_exception;
use engage_anderspink\totara_engage\resource\anderspink_articles as model_article;
use stdClass;
...
final class anderspink_articles extends provider
{
    ...
    public static function get_name(): string
    {
        ...
        return get_string('resources', 'engage_anderspink');
    }
    ...
    {
        return '{engage_anderspink_articles}';
    }
    ...
    public function can_see(array $objects): array
    {
        ...
        foreach ($objects as $object) {
            ...
            if ($results[$object->objectid] == false) {
                $results[$object->objectid] = access_manager::can_manage_engage(
                    context::instance_by_id($visibility->contextid)
                );
            }
        }
    ...
    public function prime_provider_cache(): void
    {
        ...
        $builder->left_join(['engage_share', 'es'], function (builder $join): void {
        ...
        $builder->join(['user', 'out_user'], function (builder $join): void {
            $join->where_field('er.userid', 'out_user.id');
            $join->where('out_user.deleted', 0);
            $join->where('out_user.confirmed', 1);
        });

        $builder->left_join(['engage_share_recipient', 'esr'], function (builder $join): void {
        ...
        if (!empty($CFG->tenantsenabled)) {
            ...
            if (null !== $tenant_id) {
                $builder->join(['user', 'u'], function (builder $join) use ($CFG, $tenant_id): void {
                    ...
                    if (empty($CFG->tenantsisolated)) {
                        ...
                        $join->where_raw('(u.tenantid = :tenant_id OR u.tenantid IS NULL)', ['tenant_id' => $tenant_id]
                        );
                    } else {
                ...
                if (!empty($tenant_ids)) {
                    ...
                    $builder->join(['user', 'u'], function (builder $join) use ($in_sql, $parameters): void {
                        $join->where_field('er.userid', 'u.id');
                        $join->where_raw(
                    ...
                    $builder->join(['user', 'u'], function (builder $join): void {
                        $join->where_field('er.userid', 'u.id');
    ...
    public function get_all_objects_sql(): array
    {
        $sql = '
            SELECT art.id as objectid, 
                   ...
                   con.id as contextid
            FROM {engage_anderspink_articles} art
            JOIN {engage_resource} res 
                ON res.instanceid = art.id
                AND res.resourcetype = \'engage_anderspink\'
            JOIN "ttr_user" u
                ON res.userid = u.id
    ...
    public function get_details_link(int $objectid)
    {
        ...
        $resourceid = $DB->get_field('engage_resource', 'id', [
                'resourcetype' => 'engage_anderspink',
                'instanceid'   => $objectid,
            ]);

        $article_destination = link_builder::to('engage_anderspink', ['id' => $resourceid]);
