...
final class reportedcontent_watcher {
    ...
    public static function get_content(get_review_context $hook): void {
        ...
        if ('engage_anderspink' !== $hook->component || !in_array($area, $valid_areas)) {
            return;
        }

        if ($hook->area === '') {
            // It's the article itself
            $article = anderspink_articles::from_resource_id($hook->item_id);
    ...
    public static function delete_article(remove_review_content $hook): void {
        ...
        if ('engage_anderspink' !== $hook->review->get_component() || '' !== $hook->review->get_area()) {
            return;
        }
        ...
        $article = anderspink_articles::from_resource_id($hook->review->get_item_id());
        $article->delete();
